...
export const Navigate = () => {
  ...
  useEffect(() => {
    ...
    else setinputGroup(groupIDs.sort(({ name: a }, { name: b }) => a.localeCompare(b)));
    ...
    <>
      <nav className="navbar navbar-expand-lg navbar-light bg-light d-flex align-items-sm-center">
        <div className="mx-1 my-1 d-flex w-20">
          ...
          {data.id ? (
            <>
              <a href="/callback-vk" className="btn bnt-text">
            ...
            <button className="btn btn-sm btn-primary m-2" onClick={handlerOAuthVK}>
              Войти
            </button>
          )}
        </div>
                                ...
                                <select
                                  ...
                                  onChange={(e) => {
                                    setSelectInputGroup({
                                      ...
                                      name: e.target.options[e.target.selectedIndex].getAttribute("data-name"),
                                      dateEnd: new Date(Date.now()).toLocaleDateString("en-CA"),
                                      dateStart: new Date(Date.now()).toLocaleDateString("en-CA"),
                                      photo_100: "",
                                    });
                                  ...
                                  {inputGroup.map((item) => (
                                    <option key={item.id} value={item.id} data-name={item.name.toString()}>
                                      {item.name.trim()}
                                    </option>
                                  ))}
                                </select>
                          ...
                          <div className="row">
                            <div className="col-12 my-1 d-flex p-0">
                              <input
                                ...
                                defaultValue={new Date(Date.now()).toLocaleDateString("en-CA")}
                                onChange={(e) =>
                                  setSelectInputGroup({
                                    ...selectInputGroup,
                              ...
                              />
                              <span className="mx-1 d-flex align-items-center">по:</span>
                              <input
                                className="form-control form-control-sm d-inline"
                                style={{ width: "auto" }}
                                type="date"
                            ...
                            <div className="col-12 p-0">
                              <div className="">
                                <button className="btn btn-primary btn-sm my-2" onClick={handlerGetStatGroup}>
                                  Загрузить аналитику
                                </button>
                              </div>
                            </div>
